...
export const SCENE3D_SYSTEM_INSTRUCTION = [
  ...
  "geometry.kind is one of: box, roundedBox, sphere, cylinder, cone, plane, torus, torusKnot, capsule, icosahedron, lathe, extrude, tube. args is the THREE.js geometry args array (optional; sensible defaults applied).",
  "PREFER roundedBox over box for almost all real objects (furniture, electronics, books, appliances, packaging) — sharp box corners look unfinished; real objects have slight bevels. roundedBox args:[width,height,depth,cornerRadius,smoothness] — keep cornerRadius small (~0.02–0.08 of the smallest side).",
  "USE lathe for any smooth object with rotational symmetry: vases, bottles, glasses, cups, bowls, plates, lamp bases/shades, bowling pins, chess pieces, wheels, columns, finials, doorknobs. A lathe revolves a 2D profile around the Y axis. Provide geometry:{ kind:\"lathe\", points:[[radius,height],...], segments? }. Each point is [radiusFromAxis(>=0), height]; list them from bottom to top. Start and end at radius 0 to close the form (e.g. a solid vase top), or step back inward to model a hollow opening. Example wine glass: points:[[0,0],[0.25,0],[0.25,0.04],[0.04,0.1],[0.04,0.55],[0.18,0.62],[0.22,0.8],[0.2,0.82],[0.04,0.66],[0.04,0.1]].",
  "USE extrude for flat shapes with thickness — gears, cogs, stars, signs, plaques, letters, keys, picture frames, flat brackets. Provide geometry:{ kind:\"extrude\", shape:[[x,y],...], depth?, bevel? }. shape is a closed 2D cross-section (XY plane, centred near origin); depth is the thickness; a small bevel (~0.02) softens edges. Rotate via the node transform to stand it upright.",
  "USE tube for anything that follows a curved path — pipes, hoses, cables, wires, handles, vines, tubular frames, garden hoses. Provide geometry:{ kind:\"tube\", path:[[x,y,z],...], radius? }. path is a list of 3D points the tube smoothly sweeps through; radius is its thickness.",
  "material fields: type?:\"standard\"|\"physical\"|\"basic\", color, roughness(0..1), metalness(0..1), emissive(hex), emissiveIntensity, opacity(0..1), transmission(0..1, needs type physical for glass), ior, thickness, clearcoat(0..1, glossy lacquer over the base — car paint, varnished wood, polished plastic), clearcoatRoughness, sheen(0..1, soft fabric/velvet/satin glow), sheenColor, wireframe, flatShading. clearcoat/sheen imply a physical material.",
